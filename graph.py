...
def normalize_relation(relation: str) -> str:
    '''
    Normalize a relation name to a unified format.
    Some datasets prefix relations with '+' (forward) or '-' (inverse).
    This converts them to the _reverse suffix convention:
      - '+foo' -> 'foo'
      - '-foo' -> 'foo_reverse'
    Relations already using the _reverse suffix are left unchanged.
    '''
    if relation.startswith('-'):
        return relation[1:] + '_reverse'
...
class Dataset:
    ...
    def set_id2rel(self, filename):
        ...
        self.id2rel = {k: normalize_relation(v) for k, v in id2rel.items()}
        self.rel2id = {v: k for k, v in self.id2rel.items()}
        if self.logging:
