...
class Query:
    ...
    def change_predicate(self, atom_index: int, new_relation: int):
        if atom_index < 0 or atom_index >= self.get_num_atoms():
            raise IndexError("Atom index out of range")
        self.atoms[atom_index]['relation'] = new_relation
        self.query = self.update_query_with_new_atoms()
        
    def update_query_with_new_atoms(self):
        if self.query_type in ['1p', '2p', '3p', '4p']:
            anchor = self.query[0][0]
            relations = tuple(self.atoms[i]['relation'] for i in range(len(self.atoms)))
            return ((anchor, relations),)
        elif self.query_type in ['2i', '3i', '4i']:
            branches = []
            for j in range(len(self.atoms)):
                atom = self.atoms[j]
                branch = (atom['head'], (atom['relation'],))
                branches.append(branch)
            return tuple(branches)
        elif self.query_type == '2u':
            atom1 = self.atoms[0]
            atom2 = self.atoms[1]
            branch1 = (atom1['head'], (atom1['relation'],))
            branch2 = (atom2['head'], (atom2['relation'],))
            return (branch1, branch2)
        elif self.query_type == 'up':
            atom1 = self.atoms[0]
            atom2 = self.atoms[1]
            relation3 = self.atoms[2]['relation']
            branch1 = (atom1['head'], (atom1['relation'],))
            branch2 = (atom2['head'], (atom2['relation'],))
            return (branch1, branch2, relation3)
            ...
            atom2 = self.atoms[1]
            atom3 = self.atoms[2]
            branch1 = (atom1['head'], (atom1['relation'], atom2['relation']))
            branch2 = (atom3['head'], (atom3['relation'],))
            return (branch1, branch2)
        else:
            raise ValueError(f"Unknown query type: {self.query_type}")
        
    def copy(self):
        return Query(self.query_type, (self.query, self.answer))
