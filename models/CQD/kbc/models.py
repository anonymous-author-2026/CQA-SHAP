...
class KBCModel(nn.Module, ABC):
	...
	def __get_chains__(chains: List, graph_type: str = QuerDAG.TYPE1_2.value):
		...
		if QuerDAG.TYPE1_1.value in graph_type:
		...
		elif QuerDAG.TYPE1_4.value in graph_type:
			lhs_1 = chain1[0]
			rel_1 = chain1[1]
			...
			raw_chain = [lhs_1, rel_1, rel_2, rel_3, rel_4]

		elif QuerDAG.TYPE2_4.value in graph_type:
			lhs_1 = chain1[0]
			rel_1 = chain1[1]
			...
			raw_chain = [lhs_1, rel_1, lhs_2, rel_2, lhs_3, rel_3, lhs_4, rel_4]
...
class CP(KBCModel):
	...
	def get_full_embeddigns(self, queries: torch.Tensor):
		...
		if torch.sum(queries[:, 0] >= 0).item() > 0:  # Changed condition
			lhs = self.embeddings[0](queries[:, 0])
		else:
			lhs = None

		if torch.sum(queries[:, 1] >= 0).item() > 0:  # Changed condition
			rel = self.embeddings[1](queries[:, 1])
		else:
			rel = None

		if torch.sum(queries[:, 2] >= 0).item() > 0:  # Changed condition
